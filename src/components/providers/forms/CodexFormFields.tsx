...
export function CodexFormFields({
  ...
  const [isFetchingCatalogModels, setIsFetchingCatalogModels] = useState(false);
  ...
  const handleFetchModels = useCallback(() => {
    ...
    setIsFetchingCatalogModels(true);
    fetchModelsForConfig(codexBaseUrl, codexApiKey, isFullUrl)
      ...
      .finally(() => setIsFetchingCatalogModels(false));
  ...
  const renderCatalogActionButtons = (onAdd: () => void, addLabel: string) => (
    <div className="flex gap-1">
      <Button
        ...
        disabled={isFetchingCatalogModels}
        className="h-7 gap-1"
      >
        {isFetchingCatalogModels ? (
          ...
          <div className="flex items-center gap-2">
            <div className="min-w-0 flex-1">
              <ModelSuggest
                id="codexModelName"
                value={modelName}
                onChange={(value) => onModelNameChange(value)}

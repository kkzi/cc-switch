...
基于 [farion1231/cc-switch](https://github.com/farion1231/cc-switch) 的长期 fork；截至 2026-04-05，已合并上游 `main`。
...
这个仓库不是“只打几个 patch”的镜像，而是一个长期跟随 upstream 的深度 fork。相对上游，当前主要差异集中在下面几条主线：

- 代理、路由与故障转移扩展：在上游代理能力基础上继续扩展了模型级路由、模型级健康状态、模型级故障转移队列、混合故障转移链，以及代理实际生效供应商的前端标识。冲突热点主要在 `src-tauri/src/proxy/`、`src-tauri/src/database/dao/fork_proxy.rs` 及相关 `fork_` 数据表。
- 托管认证与 GitHub Copilot 集成：增加了 Auth Center、GitHub Copilot OAuth、多账号选择、供应商绑定托管认证源、Copilot 专用转发与用量查询。这部分主要落在 `src-tauri/src/commands/auth.rs`、`src-tauri/src/commands/copilot.rs`、`src-tauri/src/proxy/providers/copilot_auth.rs`、`src/components/settings/AuthCenterPanel.tsx`。
- 主窗口、托盘与 deep link 生命周期：保留了本 fork 自己的窗口策略，包括托盘左键切换显示/隐藏、Windows 任务栏显示控制、主窗口“先隐藏再延时销毁”、deep link 在窗口未 ready 时的缓冲与重放，以及 lightweight mode 相关逻辑。热点文件在 `src-tauri/src/main_window.rs`、`src-tauri/src/lib.rs`、`src-tauri/src/tray.rs`、`src-tauri/src/lightweight.rs`。
- 供应商表单与列表交互：相对上游，本 fork 额外维护了一批面向自用的 UI / UX 调整，包括供应商卡片信息密度、模型建议输入、剪贴板快速新增、测试/测速工作流、若干 preset 与表单布局差异。热点主要在 `src/components/providers/`、`src/components/providers/forms/`、`src/config/`。
- fork 数据隔离策略：尽量把 fork 自己的路由、健康状态、设置与同步逻辑放在独立命名空间、独立附加表或独立模块里，减少和上游 schema 迁移的直接耦合。同步时不建议简单把这部分改动回滚到上游实现。

上面不是完整清单，但已经覆盖了后续从 upstream 同步时最容易冲突、最不适合直接“以上游为准”覆盖的区域。
...
- **代理与故障转移能力更激进**：除了上游已有能力外，本 fork 长期保留模型级路由、健康状态、故障转移链与供应商状态可视化相关扩展。
- **认证中心与 Copilot 支持更重**：这里把 GitHub Copilot 当作一等公民来维护，包含 OAuth、托管认证绑定、专用代理转发和用量查询。
- **窗口 / 托盘 / deep link 行为不同**：本 fork 更偏“桌面工具化”使用习惯，窗口显示隐藏、任务栏可见性、deep link 回放、lightweight mode 都和上游不完全一致。
